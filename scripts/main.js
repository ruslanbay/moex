...
function getTicketList() {
  ...
  const url = `data/securities-by-sector.tsv?_=${new Date().toISOString().split('T')[0]}`;
  const columnIndex = 1;
  const tickerList = [];
  fetch(url)
    .then(response => {
...
async function prepHistogramData() {
  ...
  try {
    ...
    let response = await fetch(`data/securities-by-sector.tsv?_=${new Date().toISOString().split('T')[0]}`);
...
async function prepTreemapData() {
  ...
  const rows = await fetch(`data/securities-by-sector.tsv?_=${new Date().toISOString().split('T')[0]}`)
...
async function loadData() {
  ...
  const response = await fetch(`data/securities-by-sector.tsv?_=${new Date().toISOString().split('T')[0]}`);
